function AC_PSD_analysis = Chen_AC_PSD(outputFolder, startTime, endTime,...
                                       binSize, cellsToPlot, maxLag,...
                                       localMaxWidth,showPlots, binMatrix)
...
% OUTPUT
%   AC_PSD_Analysis = an array of struct, one struct for each cell. Each struct
%       has five fields:
...
%       time).
%   PSD_Freq_Amps simply has data that was plotted in PSD. Column 1
%       is x values (ie the frequencies) and Column 2 is y values
%       (ie the amplitude associated with each frequency).
%   PSD_MaxFreqs_MaxAmps: Column 1 has the 10 frequencies with greatest
...
AC_PSD_analysis = cell(numCells+1,1);
...
%POWER SPECTRAL DENSITY ESTIMATE
    %power spectrum
    F = periodogram(data,rectwin(length(data)),length(data), 1/binSize);
    F = F(1:end-1);
    ...
    %plot the PSD
    PSD_name = strcat('PSD_Cell_',num2str(c));
...
% OUTPUT
    ...
    if ~strcmpi(outputFolder, 'dont save')
        ...
        saveas(PSD_plot, strcat(target_folder, '/', PSD_name, '.fig')); 
    end
    ...
    s = struct('AC_Time_Corr', [auto_x' r_vector'],...
               ...
               'PSD_Freqs_Amps', [freqs' F],...
               'PSD_MaxFreqs_MaxAmps', [maxFreqs' maxAmps]);
    AC_PSD_analysis{c} = s;        
end

    fourHzTop3 = sortrows(fourHzTop3, 2, 'descend');
    s = struct('Cell_4HzScore', fourHzTop3);
    AC_PSD_analysis{numCells + 1} = s;
    
%save the output analysis
if ~strcmpi(outputFolder, 'dont save')
    save( strcat(target_folder,'/AC_PSD_analysis.mat'), 'AC_PSD_analysis');
end
