...
spikeTime = clusterData.stamps';

spikesInWindow = null(length(spikeTime));

% for each stim
for thisStim = 1:numStims;
    
    % look at each spike
    for thisSpike = 1:length(spikeTime);
        
        % and if the spike is within the stim period, add it to
        % spikesInWindowt
        if thisStim == numStims;
        elseif (spikeTime(thisSpike) >= stimOnTimestamps(thisStim) & spikeTime(thisSpike) < stimOnTimestamps(thisStim+1)) == 1;
           % turn spike stamp into ms value
           tempStamp = spikeTime(thisSpike) - stimOnTimestamps(thisStim); 
           disp(thisSpike)
