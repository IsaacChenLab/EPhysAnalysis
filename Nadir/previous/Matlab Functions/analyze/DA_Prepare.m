...
f=find(fl=='\');
ffl=[fl(1:f(end-1)) 'BHV' fl(f(end):end)];
if fl(2)==':'
    nfl=[fl(1:end-4) '_prepared.mat'];
else
    nfl=['d:\Work\Project 1\Dopamine\' fl(1:end-4) '_prepared.mat'];
end
s=size(SWEEP_DATA);
...
while rep
    hold off;
    plot (c1(1:10)-c1(pos),1,'kx');
    hold on;
    plot (c2(1:10)-c2(1),1,'ro');
    p=ginput(1);
    if isempty(p)
        rep=0;
