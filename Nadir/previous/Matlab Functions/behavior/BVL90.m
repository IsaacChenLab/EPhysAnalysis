...
D1=5000;D2=7000;
rand_times_horizlines=D1+floor((D2-D1)*rr);
k=rand_times_horizlines;
...
while run_ind,    
        ...
        t(find(t==0))=10;
        t=t-10;
        oriz,for i=1:k, axis([1 5 t(i) t(i)+10]);, pause(0.01);,end,close %16s
        LICKS=0;
        ...
        t(find(t==0))=10;
        t=t-10;
        oriz1,for i=1:167, axis([1*cos(63) 2*cos(63) t(i)*sin(63) (t(i)+10)*sin(63)]);, pause(0.01);,end,close %4s
        if LICKS==1;% licks during sloping lines
        %pause(dly);
        CARD('send_dio',20);
        CARD('reward',RWD);
        end %no licks, no reward, we continue to next trial    
        pause(10);
        CARD('send_dio',22);
        f1; pause(90);
        CARD('send_dio',23);
        ...
        t(find(t==0))=10;
        t=t-10;
        vert,for i=1:6000, axis([t(i) t(i)+10 0 10]);, pause(0.01);,end,close
        pause(10);
        pause(d);
        d=d+1;
end
