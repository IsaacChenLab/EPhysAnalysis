function status=BHV_workMem(STATE,ITI,ISI,PRESI,TRACEI,W,NCS)
% status=BHV_workMem(STATE,ITI,ISI,PRESI,TRACEI,P,NCS)
...
if nargin<0 | isempty(STATE) | STATE<0, % resets the random ststae
    STATE=sum(100*clock);
end
...
if nargin<6 | isempty(W), % proportion of cs+
    W=0.5;
end
...
while run_ind,
    ...
    if mod(d,10)==0
        [stimuli,Minus]=BHV_workMem_genStimuli(stimuliSet,10,W);
        d=0;
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [stimuli,Minus]=BHV_workMem_genStimuli(stimuliSet,N,W)
